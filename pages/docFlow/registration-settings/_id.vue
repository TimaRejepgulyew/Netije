<template>
  <div>
    <div>
      <Header :isNew="false" :isbackButton="true" :headerTitle="registrationSetting.name"></Header>
...
export default {
  ...
  async asyncData({ app, params }) {
    var res = await app.$axios.get(
      dataApi.docFlow.RegistrationSetting + "/" + params.id
    );
    return {
      registrationSetting: res.data
    };
  },
  data() {
    return {
  ...
  methods: {
    handleSubmit() {
      ...
      this.$awn.asyncBlock(
        this.$axios.put(
          `${dataApi.docFlow.RegistrationSetting}/${this.registrationSetting.id}`,
