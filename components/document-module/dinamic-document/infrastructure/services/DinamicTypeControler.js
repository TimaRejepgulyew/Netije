...
class DinamicTypeControler {
  static generateStore(context, documentType) {
    let overlay =
      context.$store.getters[
        `dinamicDocumentComponents/${documentType}/getOverlays`
      ];
    if (overlay) {
      context.$store.commit(
        `dinamicDocumentComponents/${documentType}/IncrementOverlays`
      );
    } else {
      dinamicTypeStoreModule.registerModule(context, documentType);
      if (documentType !== "constructor") {
        context.$store.dispatch(
          `dinamicDocumentComponents/${documentType}/get_dinamic_type`,
          documentType
        );
      }
    }
  ...
  static getElements(context, documentType) {
    return context.$store.getters[
      `dinamicDocumentComponents/${documentType}/getAllElements`
    ];
  }
  static generateId() {
  ...
  static getElementById(context, documentType, elementId) {
    return context.$store.getters[
      `dinamicDocumentComponents/${documentType}/getElementById`
    ](elementId);
  }
  static removeElement(context, documentType, elementId) {
    context.$store.commit(
      `dinamicDocumentComponents/${documentType}/RemoveElement`,
      elementId
    );
  }
  static changeElement(context, documentType, newElement) {
    context.$store.commit(
      `dinamicDocumentComponents/${documentType}/ChangeElement`,
      newElement
    );
  }
  static removeStore(context, documentType) {
    ...
    if (overlay) {
      context.$store.commit(
        `dinamicDocumentComponents/${documentType}/DecrementOverlays`
      );
    } else {
  ...
  static addNewElement(context, documentType, elementId) {
    ...
    if (elementId) {
      context.$store.commit(
        `dinamicDocumentComponents/${documentType}/IntroduceElement`,
        { id: elementId, payload: newItem }
      );
    } else {
      context.$store.commit(
        `dinamicDocumentComponents/${documentType}/AddNewElement`,
        newItem
      );
    }
  }
  static async saveType(context, storeId) {
    if (storeId === "constructor") {
      await context.$store.dispatch(
        `dinamicDocumentComponents/${storeId}/create_dinamic_type`
      );
    } else {
      await context.$store.dispatch(
        `dinamicDocumentComponents/${storeId}/change_dinamic_type`
      );
    }
  }
}
